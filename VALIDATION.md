...
## September 12: 0.4.0 release and Homebrew deployment

Release [v0.4.0](https://github.com/yetidevworks/ysync/releases/tag/v0.4.0) was published from `65d098e93353fb29db0f229351e73bae55ef9df6`. Main CI passed on Ubuntu 22.04 and macOS, including native service lifecycle tests; all four release targets passed tests, Cargo installation and packaging. The release workflow updated the Homebrew tap. Homebrew upgrades and formula tests passed on the actual Mac and Linux machines.

Both services were stopped before backing up configuration and SQLite indexes; previous binaries were retained. The three configured folders (`grav`, `trilbymedia`, `yetidevworks`) now use send-only on the Mac and receive-only on Linux. Configuration comparison confirmed that only direction behavior changed; previously implicit lane/cache/retention defaults were serialized explicitly without changing their effective values. Existing files, indexed history and retained conflicts were preserved.
