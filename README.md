...
Private [GitHub Releases](https://github.com/yetidevworks/ysync/releases) provide versioned `.tar.gz` archives and `SHA256SUMS` for:
...
After authenticating `gh` with an account that can access the repository, for example on an Apple Silicon Mac:

```sh
gh release download v0.1.0 --repo yetidevworks/ysync \
  --pattern 'ysync-v0.1.0-aarch64-apple-darwin.tar.gz' --pattern SHA256SUMS
...
```sh
git clone git@github.com:yetidevworks/ysync.git
cd ysync
...
Local evaluation binaries in `dist/` remain ignored by Git. Release archives are generated by `scripts/package_release.py` and stored in private GitHub Releases.
