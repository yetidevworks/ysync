...
For the latest main branch, replace `--tag v0.3.2` with `--branch main`. To update an installed service: stop it, install the desired tag with `--force`, and start it again. Reinstall the service definition if the binary's installation path changes.
...
```sh
curl -fLO https://github.com/yetidevworks/ysync/releases/download/v0.3.2/ysync-v0.3.2-aarch64-apple-darwin.tar.gz
curl -fLO https://github.com/yetidevworks/ysync/releases/download/v0.3.2/SHA256SUMS
shasum -a 256 ysync-v0.3.2-aarch64-apple-darwin.tar.gz
# Compare the result with its matching entry in SHA256SUMS.
tar -xzf ysync-v0.3.2-aarch64-apple-darwin.tar.gz
mkdir -p ~/.local/bin
install -m 755 ysync-v0.3.2-aarch64-apple-darwin/ysync ~/.local/bin/ysync
```
